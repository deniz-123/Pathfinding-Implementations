...
class Node:
    ...
    def update_kruskal_neighbors(self, grid):

        if self.color == White:
            if self.col > 1 and not grid[self.row][self.col - 2].color == White:  # UP
                self.kruskal_neighbors.remove(grid[self.row][self.col - 2])

            if self.col < self.t_cols - 2 and not grid[self.row][self.col + 2].color == White:  # DOWN
                self.kruskal_neighbors.remove(grid[self.row][self.col + 2])

            if self.row < self.t_rows - 2 and not grid[self.row + 2][self.col].color == White:  # RIGHT
                self.kruskal_neighbors.remove(grid[self.row + 2][self.col])

            if self.row > 1 and not grid[self.row - 2][self.col].color == White:  # LEFT
                self.kruskal_neighbors.remove(grid[self.row - 2][self.col])
